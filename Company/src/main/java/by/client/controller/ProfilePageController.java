...
public class ProfilePageController implements Initializable {
    ...
    public void clickedSubmitUserButton(ActionEvent event) throws IOException {
        if (!ValidationUtils.validateName(firstNameField.getText())) {
            showMessage("Ошибка валидации имени: допустимы только буквы, длина от 2 до 15 символов.", "error");
            return;
        }
        if (!ValidationUtils.validateSurname(lastNameField.getText())) {
            showMessage("Ошибка валидации имени: допустимы только буквы, длина от 2 до 15 символов.", "error");
            return;
        }
        if (!ValidationUtils.validateLogin(loginField.getText())) {
